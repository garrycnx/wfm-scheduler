...
if run:
    ...
    for ag in agents:
        ...
        for i,wd in enumerate(WEEKDAYS):
            ...
            # --------------------------------------------
            # NEW BREAK RULES:
            # 1. Break 1 must occur within first 180 mins.
            # 2. Lunch must be at least 120 mins after Break 1.
            # 3. Break 2 must be at least 120 mins after Lunch.
            # --------------------------------------------

            # 1️⃣ Candidate lunch slots (60 min)
            lunch_cands = [t for t in slot_candidates if t+30 in slot_candidates]

            # Choose lunch slot with highest slack
            best_l = None
            best_sc = -1e9
            ...
            # 2️⃣ BREAK 1 — must be within first 180 mins of shift
            break1_limit = s + 180  # 3 hours mark
            break1_slots = [t for t in slot_candidates if t < break1_limit]

            if not break1_slots:
                break1_slots = slot_candidates[:3]

            best_b1 = None
            best_sc = -1e9
            for t in break1_slots:
                sc = slot_slack.get(min_to_time(t),0)
                if sc > best_sc:
                    best_sc = sc
                    best_b1 = t

            # 3️⃣ BREAK 2 — must be at least 120 mins after lunch
            break2_min_start = lunch_end + 120
            break2_slots = [t for t in slot_candidates if t >= break2_min_start]

            if not break2_slots:
                break2_slots = slot_candidates[-3:]

            best_b2 = None
            best_sc = -1e9
            for t in break2_slots:
                sc = slot_slack.get(min_to_time(t),0)
                if sc > best_sc:
                    best_sc = sc
                    best_b2 = t
